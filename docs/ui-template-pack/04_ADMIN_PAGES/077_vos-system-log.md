...
Expose VOS operational log read-only.
...
|---|---|---|
| F01 | Type Information/General/Error | Required |
| F02 | record time | Required |
| F03 | operating user | Required |
| F04 | event | Required |
| F05 | detail | Required |
| F06 | serial number | Required |
| F07 | link to related resource when identifiable | Required |

## VOS / Product Basis

VOS System Log fields are documented. [VOS]/[VERIFY-API]
...
- `page_viewed` with page=`vos-system-log` and side=`admin`.
