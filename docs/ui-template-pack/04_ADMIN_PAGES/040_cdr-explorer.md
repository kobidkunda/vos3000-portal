...
Global historical CDR query and export.
...
|---|---|---|
| F01 | Filters: start/end time mode, caller, callee, account, agent, mapping/routing gateway, caller/callee IP, call type, area/prefix, duration, charge, expense, termination reason, hangup side, Call-ID, serial number | Required |
| F02 | Columns selectable/persisted | Required |
| F03 | Server-side pagination | Required |
| F04 | Saved views | Required |
| F05 | CSV/XLSX export job for large ranges | Required |
| F06 | Aggregate footer totals | Required |
| F07 | No unrestricted wildcard export for non-privileged roles | Required |

## VOS / Product Basis

VOS CDR documents caller/callee, times, actual/charged duration, charge/expense, gateways, IPs, account/agent, call type, rewrite numbers, Call-IDs, reason and serial. [VOS]/[HYBRID]
...
- `page_viewed` with page=`cdr-explorer` and side=`admin`.
