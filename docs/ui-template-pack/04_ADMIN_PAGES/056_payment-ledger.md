...
|---|---|
| Side | **Admin** |
| Product group | **Billing, Payments & Settlement** |
...
All customer payments and portal payment-processor events.
...
|---|---|---|
| F01 | Filters account/customer/type/mode/status/date/amount | Required |
| F02 | VOS payment amount and resulting balance | Required |
| F03 | Portal processor transaction ID | Required |
| F04 | Payment/Credit/Create Account categorization where available | Required |
| F05 | Manual adjustment distinction | Required |
| F06 | Reconciliation state | Required |
| F07 | Receipt link | Required |
| F08 | Export | Required |

## VOS / Product Basis

VOS Payment Record documents account, amount, balance after payment, type, time, mode, payment user, memo, agent and serial. Portal reconciliation is [HYBRID].
...
- High-risk mutation requires explicit action permission, confirmation and audit event; re-authentication where configured.
...
- `page_viewed` with page=`payment-ledger` and side=`admin`.
