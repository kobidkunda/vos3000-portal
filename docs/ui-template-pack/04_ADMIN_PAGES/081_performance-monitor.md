...
|---|---|---|
| F01 | CPU/memory/process/load metrics only where available | Required |
| F02 | VOS operation performance data | Required |
| F03 | portal adapter latency | Required |
| F04 | CDR ingest lag | Required |
| F05 | historical charts | Required |

## VOS / Product Basis

VOS includes Operation Performance, Process Monitor and Server Monitor. Exact metrics [VERIFY-API].
...
**Left**
- Page title: **Performance Monitor**
...
- `page_viewed` with page=`performance-monitor` and side=`admin`.
