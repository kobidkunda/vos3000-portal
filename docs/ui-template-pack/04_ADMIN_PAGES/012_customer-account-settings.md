...
Edit VOS-account-facing commercial and lifecycle settings.
...
|---|---|---|
| F01 | Account name | Required |
| F02 | VOS account mapping | Required |
| F03 | Current balance read-only | Required |
| F04 | Overdraft/credit limit | Required |
| F05 | Billing rate group | Required |
| F06 | Private rate indicator | Required |
| F07 | Expiry date | Required |
| F08 | Account status normal/locked | Required |
| F09 | Memo/internal notes | Required |
| F10 | Agent parent | Required |
| F11 | Account type/category where applicable | Required |
| F12 | Danger-zone actions: disable/enable/delete mapping | Required |

## VOS / Product Basis

These fields are documented in VOS Account Management. Writes are [VERIFY-API].
...
- `page_viewed` with page=`customer-account-settings` and side=`admin`.
