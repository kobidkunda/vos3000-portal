...
Observe network path quality per gateway.
...
| F02 | Remote IP | Required |
| F03 | Network quality | Required |
| F04 | Packet loss | Required |
| F05 | Network delay | Required |
| F06 | Device ID | Required |
| F07 | Mapping/routing type | Required |
| F08 | History chart maintained by portal | Required |
| F09 | Threshold filters | Required |
| F10 | Degraded-only toggle | Required |

## VOS / Product Basis

VOS documents Mapping Gateway Network and Routing Gateway Network. Historical chart is [HYBRID].
...
- `page_viewed` with page=`gateway-network-quality` and side=`admin`.
