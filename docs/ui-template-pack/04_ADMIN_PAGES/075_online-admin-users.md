...
Show current VOS/portal user sessions where available.
...
|---|---|---|
| F01 | User | Required |
| F02 | session source | Required |
| F03 | login time | Required |
| F04 | last activity | Required |
| F05 | IP | Required |
| F06 | portal vs VOS indicator | Required |

## VOS / Product Basis

VOS includes Online User query. Portal session data is [HYBRID].
...
- `page_viewed` with page=`online-admin-users` and side=`admin`.
