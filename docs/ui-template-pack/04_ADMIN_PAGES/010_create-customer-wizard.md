...
|---|---|
| Side | **Admin** |
| Product group | **Customers & Accounts** |
...
Controlled onboarding that creates portal identity and maps it to VOS.
...
|---|---|---|
| F01 | Step 1 company/profile | Required |
| F02 | Step 2 portal owner credentials | Required |
| F03 | Step 3 VOS account mapping or create-new request | Required |
| F04 | Step 4 billing currency, credit/overdraft, expiry | Required |
| F05 | Step 5 rate group/private-rate policy | Required |
| F06 | Step 6 gateway/channel/CPS defaults | Required |
| F07 | Step 7 permissions/API access | Required |
| F08 | Review-and-create screen | Required |
| F09 | Rollback/compensation if one downstream step fails | Required |

## VOS / Product Basis

Account creation, billing rate assignment, account status and agent hierarchy are VOS functions; exact create API is [VERIFY-API].
...
├─ validation
├─ Back / Continue
└─ final review + transactional create
```
...
**Left**
- Page title: **Create Customer Wizard**
...
- `page_viewed` with page=`create-customer-wizard` and side=`admin`.
