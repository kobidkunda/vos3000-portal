...
Manage customer-side ingress/mapping gateways.
...
|---|---|---|
| F01 | Search/filter by gateway, customer, IP, status, softswitch | Required |
| F02 | Columns: name, account, IP, registration mode, line limit, active calls, CPS, ASR, ACD, status | Required |
| F03 | Create/edit/disable | Required |
| F04 | Open live calls | Required |
| F05 | Open network quality | Required |
| F06 | Bulk export | Required |

## VOS / Product Basis

Mapping Gateway management and online metrics are documented by VOS. Writes [VERIFY-API].
...
- `page_viewed` with page=`mapping-gateways` and side=`admin`.
