...
|---|---|
| Side | **Admin** |
| Product group | **Number Management** |
...
Manage number rewrite/transform rules.
...
|---|---|---|
| F01 | Rule list | Required |
| F02 | source pattern | Required |
| F03 | target | Required |
| F04 | scope | Required |
| F05 | priority/order where supported | Required |
| F06 | test input/output | Required |
| F07 | impact preview | Required |
| F08 | audit | Required |

## VOS / Product Basis

VOS Number Transform and rewrite rules are documented. Exact fields/API [VERIFY-API].
...
- `page_viewed` with page=`number-transform` and side=`admin`.
