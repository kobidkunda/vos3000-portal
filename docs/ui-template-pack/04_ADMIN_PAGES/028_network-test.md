...
Test reachability/network condition to a remote IP.
...
| F02 | Configuration port | Required |
| F03 | Authorized local IP | Required |
| F04 | Packet type | Required |
| F05 | Run test | Required |
| F06 | Result history | Required |
| F07 | Copy diagnostics | Required |

## VOS / Product Basis

VOS Network Test supports special-format and ICMP testing. [VOS]/[VERIFY-API]
...
- `page_viewed` with page=`network-test` and side=`admin`.
