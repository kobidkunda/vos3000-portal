...
Manage internal operator identities.
...
|---|---|---|
| F01 | Login name | Required |
| F02 | display name | Required |
| F03 | portal role | Required |
| F04 | mapped VOS user where required | Required |
| F05 | status | Required |
| F06 | expiry | Required |
| F07 | last login | Required |
| F08 | last password change | Required |
| F09 | MFA state | Required |
| F10 | create/lock/disable/reset | Required |

## VOS / Product Basis

VOS user management documents login/name/type/lock/invalid time/dynamic password. Portal identity is [HYBRID].
...
- `page_viewed` with page=`admin-users` and side=`admin`.
