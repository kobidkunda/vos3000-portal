...
Secure entry point for internal operators.
...
|---|---|---|
| F01 | Email/login name and password | Required |
| F02 | Optional TOTP/MFA challenge | Required |
| F03 | Remember-device policy | Required |
| F04 | Rate-limit and temporary lockout | Required |
| F05 | Display environment/server label so operators know which VOS instance they are accessing | Required |
| F06 | Security event logging for successful/failed login | Required |

## VOS / Product Basis

VOS documents Administrator/Operator/Agent user types, lock state, invalid time, dynamic password and operation authorization. Portal authentication remains independent. [HYBRID]
...
- `page_viewed` with page=`admin-login` and side=`admin`.
