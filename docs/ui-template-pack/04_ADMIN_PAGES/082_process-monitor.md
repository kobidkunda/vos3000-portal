...
Read-only process status with controlled operational actions if supported.
...
|---|---|---|
| F01 | Process name/status | Required |
| F02 | uptime | Required |
| F03 | resource usage if available | Required |
| F04 | last restart | Required |
| F05 | alert state | Required |
| F06 | no restart button until vendor-safe API verified | Required |

## VOS / Product Basis

VOS Process Monitor exists. [VERIFY-API]
...
- `page_viewed` with page=`process-monitor` and side=`admin`.
