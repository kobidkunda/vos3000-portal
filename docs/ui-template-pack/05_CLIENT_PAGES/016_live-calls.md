...
|---|---|
| Side | **Client** |
| Product group | **Live Calls & Traffic** |
| Route | `/app/calls/live` |
| Page archetype | **LIVE_MONITOR** |
| Data mode | **realtime** |
| Primary implementation phase | **P09 — Realtime Calls, Gateway Presence & NOC Streaming** |
| Source status | Product-spec derived; VOS-specific behavior must follow the basis below |

## Goal

Customer-only real-time active sessions.
...
|---|---|---|
| F01 | Caller/callee | Required |
| F02 | customer gateway | Required |
| F03 | connect time | Required |
| F04 | duration | Required |
| F05 | PDD | Required |
| F06 | codec where allowed | Required |
| F07 | active call count vs channels | Required |
| F08 | search/filter | Required |
| F09 | auto-refresh/WebSocket status | Required |
| F10 | no disconnect button by default | Required |
| F11 | carrier routing gateway and sensitive media details redacted | Required |

## VOS / Product Basis

VOS Current Call provides the underlying live-call fields. Customer projection is [HYBRID].
...
- Authenticated customer organization member.
- Backend resolves tenant from session/token; never from arbitrary request input.
- Object ownership must be validated for every customer, gateway, payment, CDR, report, API key, webhook and ticket reference.

## Canonical Layout

```text
[Page header + LIVE connection status + last update]
├─ Realtime KPI strip
├─ Status / degradation summary
└─ Streaming table or monitor grid
   └─ row drill-down / safe action
```
...
- Use WebSocket/SSE fan-out from the portal backend; browsers do not poll VOS directly.
- Throttle/coalesce high-frequency UI updates to avoid full-table re-render storms.
...
- Backend resolves tenant from session/token; never from arbitrary request input.
- Object ownership must be validated for every customer, gateway, payment, CDR, report, API key, webhook and ticket reference.
- Client API responses are field-projected/redacted server-side.
- Do not expose carrier cost, margin, other-tenant data, raw VOS credentials or private routing topology unless an explicit product requirement grants it.

## Observability & Telemetry

- `page_viewed` with page=`live-calls` and side=`client`.
...
- `realtime_connected`, `realtime_disconnected`, `realtime_stale`.
...
- [ ] All source requirements F01..F11 implemented or explicitly deferred with approval.
