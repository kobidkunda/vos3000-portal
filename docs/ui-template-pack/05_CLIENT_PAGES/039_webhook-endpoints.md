...
|---|---|
| Side | **Client** |
| Product group | **Developer API & Webhooks** |
...
|---|---|---|
| F01 | Endpoint URL | Required |
| F02 | events such as call.completed, gateway.online/offline, balance.low, payment.completed, rate.changed | Required |
| F03 | signing secret shown once/rotatable | Required |
| F04 | enable/disable | Required |
| F05 | test delivery | Required |
...
- `page_viewed` with page=`webhook-endpoints` and side=`client`.
...
- [ ] All source requirements F01..F05 implemented or explicitly deferred with approval.
