...
|---|---|
| Side | **Client** |
| Product group | **CDR & Call History** |
| Route | `/app/cdr/{cdrId}` |
| Page archetype | **DETAIL** |
| Data mode | **query** |
| Primary implementation phase | **P05 — ClickHouse CDR Store, Rollups, Query APIs & Analytics** |
| Source status | Product-spec derived; VOS-specific behavior must follow the basis below |

## Goal

Explain one call without exposing sensitive carrier internals.
...
|---|---|---|
| F01 | Begin/end time | Required |
| F02 | caller/callee | Required |
| F03 | actual and charged duration | Required |
| F04 | customer call charge | Required |
| F05 | call type/destination/area | Required |
| F06 | customer mapping gateway | Required |
| F07 | caller IP if customer is authorized to see it | Required |
| F08 | termination reason/hangup side | Required |
| F09 | PDD/connection delay | Required |
| F10 | customer-side Call-ID/serial as appropriate | Required |
| F11 | Report Problem button | Required |
| F12 | Do not reveal carrier cost, routing gateway, other-customer data, private softswitch internals unless explicitly allowed | Required |

## VOS / Product Basis

VOS CDR contains richer carrier-side information; client response must be projection/redaction. [HYBRID]
...
```text
[Entity/page header]
├─ identity + status + key actions
├─ summary metrics / key values
├─ tabbed or sectioned detail content
├─ related records
└─ activity/audit/support context
```
...
- `page_viewed` with page=`cdr-detail` and side=`client`.
...
- [ ] All source requirements F01..F12 implemented or explicitly deferred with approval.
