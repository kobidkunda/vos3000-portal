...
|---|---|---|
| F01 | Default recent date range | Required |
| F02 | Filters: begin/end date, caller, callee, gateway, call status/termination reason, duration, destination/area, Call-ID | Required |
| F03 | Columns: begin/end, caller/callee, duration, charged duration, customer charge, mapping gateway, destination, termination reason | Required |
| F04 | Customer cannot access other accounts even by manipulating query parameters | Required |
| F05 | Saved filters | Required |
| F06 | CSV/XLSX export | Required |
| F07 | Server-side pagination | Required |

## VOS / Product Basis

VOS CDR documents the underlying fields. Tenant scoping and export UX are [HYBRID].
...
│  ├─ row status + actions
│  └─ server-side pagination
└─ bulk/action footer when permitted
```
...
- `page_viewed` with page=`cdr-explorer` and side=`client`.
