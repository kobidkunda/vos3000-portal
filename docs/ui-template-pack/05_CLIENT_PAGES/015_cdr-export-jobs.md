...
|---|---|---|
| F01 | Requested range/filter | Required |
| F02 | status queued/running/ready/expired/failed | Required |
| F03 | row count | Required |
| F04 | created/expiry time | Required |
| F05 | download when ready | Required |
| F06 | cancel queued job | Required |
| F07 | rate limit export creation | Required |
...
- `page_viewed` with page=`cdr-export-jobs` and side=`client`.
