...
|---|---|
| Side | **Client** |
| Product group | **Notifications** |
| Route | `/app/settings/notifications` |
| Page archetype | **SETTINGS** |
| Data mode | **transactional/configuration** |
| Primary implementation phase | **P11 — Customer API, Webhooks, Reports & Large Exports** |
| Source status | Product-spec derived; VOS-specific behavior must follow the basis below |

## Goal

Customer-controlled notification channels and thresholds.
...
|---|---|---|
| F01 | Low-balance amount | Required |
| F02 | gateway state alerts | Required |
| F03 | rate changes | Required |
| F04 | payment alerts | Required |
| F05 | security alerts always-on where required | Required |
| F06 | email/webhook/other channel toggles | Required |
| F07 | quiet-hours policy only for noncritical alerts | Required |
...
└─ sticky save bar + audit/change reason
```
...
- `page_viewed` with page=`alert-preferences` and side=`client`.
