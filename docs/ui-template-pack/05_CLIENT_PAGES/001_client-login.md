...
|---|---|---|
| F01 | Email/username and password | Required |
| F02 | MFA challenge when enabled | Required |
| F03 | Remember device subject to policy | Required |
| F04 | Forgot password | Required |
| F05 | Clear account-suspended/expired states without exposing sensitive backend details | Required |
...
└─ Auth card
   ...
   ├─ Form
   ├─ Primary action
   └─ Secondary recovery/help action
```
...
- `page_viewed` with page=`client-login` and side=`client`.
