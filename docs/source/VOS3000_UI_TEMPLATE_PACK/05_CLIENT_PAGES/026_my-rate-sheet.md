...
Customer's assigned sell rates.
...
|---|---|---|
| F01 | Search by country/destination/prefix | Required |
| F02 | Prefix | Required |
| F03 | area/destination | Required |
| F04 | rate type | Required |
| F05 | rate/minute | Required |
| F06 | billing cycle where customer-facing | Required |
| F07 | effective date supplied by portal rate-versioning | Required |
| F08 | download CSV/XLSX | Required |
| F09 | never expose carrier cost/private admin margins | Required |

## VOS / Product Basis

VOS Rate Management provides prefix, area, type, billing rate/cycle and rate/minute. Presentation/versioning is [HYBRID].
...
- `page_viewed` with page=`my-rate-sheet` and side=`client`.
