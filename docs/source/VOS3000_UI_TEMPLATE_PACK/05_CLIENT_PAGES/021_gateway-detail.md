...
| Data mode | **query** |
| Primary implementation phase | **P07 — Customer Self-Service Core — Dashboard, CDR, Gateways & Rates** |
| Source status | Product-spec derived; VOS-specific behavior must follow the basis below |

## Goal

Single gateway configuration and health.
...
|---|---|---|
| F01 | Gateway identity | Required |
| F02 | account | Required |
| F03 | static/dynamic mode | Required |
| F04 | configured IP(s) | Required |
| F05 | registered IP | Required |
| F06 | signaling port if exposed | Required |
| F07 | line limit | Required |
| F08 | current calls | Required |
| F09 | CPS | Required |
| F10 | ASR/ACD | Required |
| F11 | registration/update time | Required |
| F12 | network latency/loss | Required |
| F13 | softswitch host details only if safe to expose | Required |
| F14 | CDR and live-call shortcuts | Required |
| F15 | configuration actions governed by customer permission | Required |

## VOS / Product Basis

Based on VOS Mapping Gateway and Online Mapping Gateway. [HYBRID]
...
- `page_viewed` with page=`gateway-detail` and side=`client`.
...
- [ ] All source requirements F01..F15 implemented or explicitly deferred with approval.
