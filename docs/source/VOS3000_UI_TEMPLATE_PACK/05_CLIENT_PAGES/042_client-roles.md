...
|---|---|
| Side | **Client** |
| Product group | **Team & Permissions** |
| Route | `/app/team/roles` |
| Page archetype | **DETAIL** |
| Data mode | **query** |
| Primary implementation phase | **P02 — Identity, RBAC, Tenant Isolation & Audit Foundation** |
| Source status | Product-spec derived; VOS-specific behavior must follow the basis below |

## Goal

Customer-admin-managed RBAC.
...
|---|---|---|
| F01 | Owner, Billing, Technical/NOC, Read Only, API Manager | Required |
| F02 | Permissions: view CDR, view rates, view balance, add funds, manage gateway IP, view live calls, manage API, download reports, manage team | Required |
| F03 | Custom roles optionally later | Required |
...
- `page_viewed` with page=`client-roles` and side=`client`.
...
- [ ] All source requirements F01..F03 implemented or explicitly deferred with approval.
