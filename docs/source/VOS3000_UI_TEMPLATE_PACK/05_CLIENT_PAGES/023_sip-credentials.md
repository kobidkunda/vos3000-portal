...
Expose connection information without leaking reusable secrets.
...
|---|---|---|
| F01 | SIP host/address | Required |
| F02 | port | Required |
| F03 | protocol | Required |
| F04 | gateway/auth username if applicable | Required |
| F05 | password never re-displayed after creation | Required |
| F06 | Reset/rotate password action | Required |
| F07 | copy non-secret fields | Required |
| F08 | rotation confirmation and audit | Required |

## VOS / Product Basis

VOS documents configuration/authentication credentials for gateway/phone objects; secure portal handling is [HYBRID]/[VERIFY-API].
...
- `page_viewed` with page=`sip-credentials` and side=`client`.
