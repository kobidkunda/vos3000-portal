...
|---|---|
| Side | **Client** |
| Product group | **Balance, Funds & Payments** |
| Route | `/app/billing/add-funds` |
| Page archetype | **FINANCE_ACTION** |
| Data mode | **transactional** |
| Primary implementation phase | **P08 — Billing, Add-Funds, Immutable Ledger & VOS Reconciliation** |
...
|---|---|---|
| F01 | Preset and custom amount | Required |
| F02 | Currency | Required |
| F03 | Payment method selection | Required |
| F04 | Minimum/maximum validation | Required |
| F05 | Fees/expected credit disclosure | Required |
| F06 | Redirect/embedded provider flow | Required |
| F07 | Pending/success/failed states | Required |
| F08 | Idempotent payment creation | Required |
| F09 | Never credit VOS until payment is independently verified by backend webhook | Required |

## VOS / Product Basis

Online payment provider is [PORTAL]; confirmed funds can be translated into VOS Payment/Credit via adapter [VERIFY-API].
...
- Mutation requires the matching organization permission; Owner alone is not implicitly sufficient unless policy grants it.
...
├─ action form
├─ fee / before-after / expected result preview
├─ explicit confirmation
└─ processing/result timeline
```
...
- Money operations are idempotent and use decimal-safe values; browser redirects never prove payment success.

## Observability & Telemetry

- `page_viewed` with page=`add-funds` and side=`client`.
...
- [ ] All source requirements F01..F09 implemented or explicitly deferred with approval.
