...
|---|---|
| Side | **Client** |
| Product group | **Gateways & SIP** |
| Route | `/app/gateways/{gatewayId}/ips` |
| Page archetype | **EDITOR_FORM** |
| Data mode | **transactional/configuration** |
| Primary implementation phase | **P10 — Gateway, Rate, Package & Routing Configuration Workflows** |
| Source status | Product-spec derived; VOS-specific behavior must follow the basis below |

## Goal

Self-service customer IP changes with strong controls.
...
|---|---|---|
| F01 | Current primary/backup IPs | Required |
| F02 | Add/replace/remove subject to plan | Required |
| F03 | IPv4/IPv6/CIDR validation based on VOS capability | Required |
| F04 | Require MFA/re-authentication | Required |
| F05 | Optional email approval/admin review | Required |
| F06 | Show impact warning before changing active gateway | Required |
| F07 | Audit old/new IP | Required |
| F08 | Connectivity status after change | Required |

## VOS / Product Basis

VOS mapping gateways are IP-address based and can expose registered IP; self-service workflow is [HYBRID]/[VERIFY-API].
...
**Left**
- Page title: **Gateway IP Management**
...
- `page_viewed` with page=`gateway-ip-management` and side=`client`.
...
- [ ] All source requirements F01..F08 implemented or explicitly deferred with approval.
