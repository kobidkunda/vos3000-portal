...
Instant price lookup for a called number.
...
|---|---|---|
| F01 | Number input | Required |
| F02 | normalized number | Required |
| F03 | matched prefix | Required |
| F04 | destination | Required |
| F05 | rate type | Required |
| F06 | customer rate/minute | Required |
| F07 | billing increment/cycle | Required |
| F08 | clear no-rate result | Required |
| F09 | copy result | Required |

## VOS / Product Basis

VOS uses longest-prefix matching; customer lookup is [HYBRID].
...
- `page_viewed` with page=`rate-lookup` and side=`client`.
