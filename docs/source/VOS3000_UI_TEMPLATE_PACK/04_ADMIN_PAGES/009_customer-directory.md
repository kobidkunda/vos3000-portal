...
Primary customer list and lifecycle management.
...
|---|---|---|
| F01 | Search by customer name, portal ID, VOS account ID, email, phone, gateway, IP | Required |
| F02 | Filters: active/suspended, low balance, agent/general, rate group, created date | Required |
| F03 | Columns: customer, VOS account, balance, overdraft/credit, today usage, gateways, channels, status, last activity | Required |
| F04 | Bulk export | Required |
| F05 | Bulk notification | Required |
| F06 | Create customer | Required |
| F07 | Suspend/enable selected where authorized | Required |

## VOS / Product Basis

VOS General Account contains account ID/name, balance, overdraft, billing rate, today consumption, gateway/phone counts and status. [HYBRID]
...
- `page_viewed` with page=`customer-directory` and side=`admin`.
