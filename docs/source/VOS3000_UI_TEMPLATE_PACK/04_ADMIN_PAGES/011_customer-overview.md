...
|---|---|---|
| F01 | Header: status, VOS account ID, balance, credit, today/month spend, active calls | Required |
| F02 | Tabs: Overview, Billing, Gateways, Phones, Rates, CDR, Live Calls, Reports, API, Users, Tickets, Audit | Required |
| F03 | Quick actions: add funds, adjust credit, suspend, change rate, add gateway, impersonate portal user | Required |
| F04 | Risk banners: negative balance, expiring account, offline gateway, payment issue | Required |
...
- `page_viewed` with page=`customer-overview` and side=`admin`.
...
- [ ] All source requirements F01..F04 implemented or explicitly deferred with approval.
