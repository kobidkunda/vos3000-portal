...
Unified current and historical alarm workspace.
...
|---|---|---|
| F01 | Filters: severity, type, source, gateway, account, time | Required |
| F02 | Current vs historical tabs | Required |
| F03 | Acknowledge/assign/resolve workflow in portal | Required |
| F04 | Link alarm to gateway/customer/system page | Required |
| F05 | Notification policy preview | Required |
| F06 | Bulk acknowledge | Required |
| F07 | Export | Required |

## VOS / Product Basis

VOS documents system, network, disk, process, mapping, routing, balance and external-device alarms plus current/history alarms. Portal workflow is [HYBRID].
...
- `page_viewed` with page=`alarm-center` and side=`admin`.
