...
Second-factor verification for privileged users.
...
|---|---|---|
| F01 | TOTP code entry | Required |
| F02 | Recovery code | Required |
| F03 | Trusted-device option subject to policy | Required |
| F04 | Re-authentication for dangerous actions | Required |
| F05 | Clear failure/expiry states | Required |
...
- `page_viewed` with page=`mfa-challenge` and side=`admin`.
