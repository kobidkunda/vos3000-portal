...
Operational health of portal services and connected VOS components.
...
|---|---|---|
| F01 | Portal API health | Required |
| F02 | VOS adapter connectivity | Required |
| F03 | CDR ingestion lag | Required |
| F04 | Webhook queue depth | Required |
| F05 | Payment webhook health | Required |
| F06 | Background job health | Required |
| F07 | Database/storage health | Required |
| F08 | Last successful sync per resource | Required |
| F09 | Incident banner | Required |
...
- `page_viewed` with page=`system-health` and side=`admin`.
