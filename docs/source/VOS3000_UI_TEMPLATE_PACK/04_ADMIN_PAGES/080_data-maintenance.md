...
View VOS historical table volumes and retention.
...
|---|---|---|
| F01 | Tabs: system logs, history alarms, payment records, CDR, reports | Required |
| F02 | Table name/date suffix | Required |
| F03 | data volume | Required |
| F04 | memo | Required |
| F05 | retention policy | Required |
| F06 | cleanup status | Required |
| F07 | no destructive action without explicit confirmation | Required |

## VOS / Product Basis

VOS documents system-log/history-alarm/payment/CDR/report tables and automatic cleanup. [VOS]/[VERIFY-API]
...
- `page_viewed` with page=`data-maintenance` and side=`admin`.
