...
Observe DR/tolerance state and equipment.
...
|---|---|---|
| F01 | Status | Required |
| F02 | data state | Required |
| F03 | equipment | Required |
| F04 | replication/sync indicators if exposed | Required |
| F05 | last successful state | Required |
| F06 | alert integration | Required |
| F07 | read-only initial release | Required |

## VOS / Product Basis

VOS documents Disaster Tolerance Status/Data/Equipment. [VOS]/[VERIFY-API]
...
- `page_viewed` with page=`disaster-recovery` and side=`admin`.
